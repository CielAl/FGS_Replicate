...
%%% textread: Must replace the slash to double slash if use sprintf
...
for thisSet = {'train', 'val', 'test'}
  ...
  [gtids]=textread(sprintf(FuckWindows(VOCopts.imgsetpath),thisSet),'%s');
  ...
  for i=1:length(gtids)
    % Read annotation.
    rec=PASreadrecord(sprintf(FuckWindows(VOCopts.annopath),gtids{i}));
